 ...
 * {@code TechDoomDecay} ("Withering Whisper"), replacing this project's earlier vanilla-Wither stand-in.
 * Damages every {@code 40 >> amplifier} ticks (matching the original's {@code isReady} exactly) via a
 * real armor-bypassing hit, reusing {@code LivingEntity#damageSources().magic()} the same way
 ...
 * this project's pinned NeoForge source - same real gap noted on {@code DoomAbilityEvents}). Left
