...
/**
 * {@code /msu abilitech revoke user <player>} / {@code /msu abilitech grant user <player>} -
 ...
 * targets the commanding player themselves).
 * <p>
 * {@code grant} force-marks every registered tech unlocked for the target, bypassing
 * {@code canUnlock}/{@code onUnlock} entirely (spends nothing) - same real debug-bypass semantics as
 * {@code /msu unlock all}. {@code revoke} is the real inverse, not present anywhere else yet: clears the
 * target's entire unlocked set, then unequips any slot whose tech is no longer unlocked (a bare
 * {@code unlockedTechs.clear()} alone would leave an already-equipped-but-now-locked tech sitting in its
 * slot, since this project's own unlock gate only ever runs at equip time, not continuously).
 */
public final class AbilitechUserCommand
{
	...
	private static int revoke(CommandSourceStack source, ServerPlayer target) throws CommandSyntaxException
	{
	...
	private static int grant(CommandSourceStack source, ServerPlayer target) throws CommandSyntaxException
	{
		GodTierData godTier = target.getData(MSUAttachments.GOD_TIER);
