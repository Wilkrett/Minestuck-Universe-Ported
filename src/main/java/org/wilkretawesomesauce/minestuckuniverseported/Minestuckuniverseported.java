...
public class Minestuckuniverseported {
    ...
    // Creates a new Block with the id "minestuckuniverseported:example_block", combining the namespace and path
    public static final DeferredBlock<Block> EXAMPLE_BLOCK = BLOCKS.registerSimpleBlock("example_block", BlockBehaviour.Properties.of().mapColor(MapColor.STONE));
    // Creates a new BlockItem with the id "minestuckuniverseported:example_block", combining the namespace and path
    public static final DeferredItem<BlockItem> EXAMPLE_BLOCK_ITEM = ITEMS.registerSimpleBlockItem("example_block", EXAMPLE_BLOCK);

    // Creates a new food item with the id "minestuckuniverseported:example_id", nutrition 1 and saturation 2
    public static final DeferredItem<Item> EXAMPLE_ITEM = ITEMS.registerSimpleItem("example_item", new Item.Properties().food(new FoodProperties.Builder().alwaysEdible().nutrition(1).saturationModifier(2f).build()));

    // Creates a creative tab with the id "minestuckuniverseported:example_tab" for the example item, that is placed after the combat tab
    public static final DeferredHolder<CreativeModeTab, CreativeModeTab> EXAMPLE_TAB = CREATIVE_MODE_TABS.register("example_tab", () -> CreativeModeTab.builder().title(Component.translatable("itemGroup.minestuckuniverseported")).withTabsBefore(CreativeModeTabs.COMBAT).icon(() -> EXAMPLE_ITEM.get().getDefaultInstance()).displayItems((parameters, output) -> {
    ...
    public Minestuckuniverseported(IEventBus modEventBus, ModContainer modContainer) {
        ...
        org.wilkretawesomesauce.minestuckuniverseported.abilitech.MSUAbilitechs.init();
        ...
        // Note that this is necessary if and only if we want *this* class (Minestuckuniverseported) to respond directly to events.
    ...
    private void addCreative(BuildCreativeModeTabContentsEvent event) {
        ...
        if (event.getTabKey() == CreativeModeTabs.COMBAT) {
            event.accept(MSUItems.STRIFE_CARD);
            event.accept(MSUItems.GOD_TIER_HOOD);
            event.accept(MSUItems.GOD_TIER_SHIRT);
            event.accept(MSUItems.GOD_TIER_PANTS);
            event.accept(MSUItems.GOD_TIER_SHOES);
            event.accept(MSUItems.ABILITECHNOSYNTH);
        }
    }
